...
本 skill 同时供 **Claude Code / Codex / Cursor / OpenCode / pi** 使用；运行统计必须按当前 agent 分支执行，禁止混用其他环境的脚本。
...
| pi | `references/runtime_pi.md` | `scripts/pi_usage.py` |
...
两个及以上链接时，读取 `multi_video.md` 与 `pipeline.md`。协调者：`batch-init` → `batch-probe` →（门禁询问）→ 按 `batch-status` 的 `actions` 调度 summarize / download_audio / transcribe。Claude Code / Codex 每条 summarize 交给独立工作 agent；Cursor CLI、OpenCode 与 pi 在当前 turn / 会话内按输入顺序串行处理全部条目，完成一条后直接开始下一条，不向用户索要「继续」。单个失败不得中断其他条目。
